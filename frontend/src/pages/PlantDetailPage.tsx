...
import { Button, Card, Col, Descriptions, Form, Input, Modal, Popconfirm, Row, Select, Space, Typography, Upload, message } from 'antd'
...
export function PlantDetailPage() {
  ...
  const mediaSelectOptions = sortByLabel(mediaItems.map((item) => ({ value: item.id, label: item.title || item.filename })))
      ...
      <Card>
        <Descriptions bordered column={1} size='small'>
          ...
          <Descriptions.Item label='Species'>
            {plant.species?.id
              ? (
                <Button type='link' style={{ padding: 0 }} onClick={() => navigate(`/species/${plant.species?.id}`)}>
                  {plant.species.name}
                </Button>
                )
              : '-'}
          ...
          <Descriptions.Item label='Tags'>
            <Space wrap>
              {plant.tags.length > 0
                ? plant.tags.map((item) => (
                  <Button key={item.id} type='link' style={{ padding: 0 }} onClick={() => navigate(`/tags/${item.id}`)}>
                    {item.name}
                  </Button>
                ))
                : '-'}
